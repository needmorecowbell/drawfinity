...
- [x] **Check coverage and decide**: Coverage is **92.3%** (144/156 exports documented). This exceeds the 90% target. No PENDING auto-documentable items remain in the plan (all 52 items are IMPLEMENTED). **Decision: EXIT — target reached. Do NOT reset documents 1-4.**
...
### Remaining Undocumented (12 items — not blocking 90% target)
- `class ActionRegistry` (ui/ActionRegistry.ts) — low priority, internal wiring
- `class DrawDocument` (model/Document.ts) — superseded by Yjs CRDT layer
- `const ICONS` (ui/ToolbarIcons.ts) — self-explanatory SVG icon map
- `const STROKE_FRAGMENT_SHADER` / `STROKE_VERTEX_SHADER` (renderer/ShaderProgram.ts) — shader source strings
- `const TURTLE_EXAMPLES` / `interface TurtleExample` (turtle/TurtleExamples.ts) — example data
- `function computeContentBounds` (ui/ExportRenderer.ts) — utility
- `function generateStrokeId` (model/Stroke.ts) — trivial ID generator
- `interface EraserConfig` (tools/EraserTool.ts) — simple config
- `interface ToolbarOverflowConfig` (ui/ToolbarOverflow.ts) — simple config
- `interface TurtlePanelCallbacks` (ui/TurtlePanel.ts) — callback interface
