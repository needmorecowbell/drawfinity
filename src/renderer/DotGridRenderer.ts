...
/**
 * Compute the relative luminance of a hex color (sRGB).
...
/**
 * Returns the appropriate dot grid color for a given background hex color.
 * Light backgrounds get dark dots; dark backgrounds get light dots.
 */
export function autoContrastDotColor(backgroundHex: string): [number, number, number, number] {
  return hexLuminance(backgroundHex) > 0.5
    ? [0.0, 0.0, 0.0, 0.15]
    : [1.0, 1.0, 1.0, 0.2];
}
...
export class DotGridRenderer {
  ...
  /** Dot color — auto-adjusted for background contrast. */
  private dotColor: [number, number, number, number] = [0.0, 0.0, 0.0, 0.10];
  ...
  setDotColor(color: [number, number, number, number]): void {
    this.dotColor = color;
  }
